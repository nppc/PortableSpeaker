...
// 0dB = 256
const uint8_t dBlogTable[] PROGMEM = {
	228,	//-1dB
	203,	//-2dB
	181,	//-3dB
	162,	//-4dB
...
void digPot_sendCommand(uint8_t cmd, uint16_t val){
	uint8_t command;
	command = cmd;
...
void digPot_init(){
	uint8_t tcon0val = 0;
	uint8_t tcon1val = 0;
	//tcon0val = digPot_readCommand(TCON0);	// get current value
	...
	Wire.beginTransmission(MCP_I2C_ADDR);
	Wire.write(TCON1);
	Wire.write(tcon1val); // all pins are connected
	Wire.endTransmission();  
	delay(1);
